...
<?php session_start() ?>
	...
	/* estadisticas*/
					...
					$instruccion = "SELECT SUM(amam+amaz) AS amarillas  FROM incidencias 
									WHERE usuario='".$_GET['uskey']."' AND(amam>0 OR amaz>0)
...
?>
	...
	<?php
					...
					mysql_set_charset('utf8');
					$instruccion = "SELECT p.fecha, p.local, p.visitante, p.tgolam, p.tgolaz,i.color, i.amam, i.amaz
									FROM partido p
									LEFT JOIN incidencias i ON i.fecha = p.fecha
									WHERE i.usuario = '".$_GET['uskey']."'
									AND (amam >0 OR amaz >0)
...
if ($nfilas==0) {
	?>
	<h2 style="text-align:center;margin-top:150px;margin-bottom:230px;color:AntiqueWhite;">Aún no tiene ninguna tarjeta amarilla.</h2>
	<?php
