...
function Pagina(nropagina,y,z){
 ...
 ajax.open("GET", "./verpartidosjugados.php?a=1&pag="+nropagina+"&uskey="+y+"&temporadamayor="+z);
 ...
 ajax.send(null)
	...
	<div id="con" style="width:730px;">
	<div  id="todosgoles" style="background-color:black;">
<?php
/*
$_GET['uskey'];
$_GET['temporadamayor']
...
{ session_start(); }
 ...
 if(isset($_GET['pag'])){
  ...
  $PagAct=1;
					...
					WHERE i.usuario = '".$_GET['uskey']."' AND i.fecha<='".$_GET['temporadamayor']."/06/30' AND i.fecha>='".($_GET['temporadamayor']-1)."/09/01'
	...
	<?php
					...
					$instruccion = "SELECT i.usuario,i.golam, i.roam, i.amam, s.nombre, s.apellidos, s.posicion, i.color
									...
									AND color = 'am' AND i.fecha<='".$_GET['temporadamayor']."/06/30' AND i.fecha>='".($_GET['temporadamayor']-1)."/09/01'
					...
					$instruccion = "SELECT i.usuario,i.golaz, i.roaz, i.amaz, s.nombre, s.apellidos, s.posicion, i.color
									...
									LEFT JOIN socio s ON i.usuario = s.usuario
									WHERE fecha = '".$resultado['fecha']."'
									AND color = 'az' AND i.fecha<='".$_GET['temporadamayor']."/06/30' AND i.fecha>='".($_GET['temporadamayor']-1)."/09/01'
									ORDER BY posicion ASC";
...
<?php	
	...
	include ('../../php/conexion.php');
					...
					FROM incidencias
					WHERE fecha = '".$resultado['fecha']."'
					AND color = 'az' AND fecha<='".$_GET['temporadamayor']."/06/30' AND fecha>='".($_GET['temporadamayor']-1)."/09/01'
					GROUP BY color";
	...
	include ('../../php/conexion.php');
					...
					FROM incidencias
					WHERE fecha = '".$resultado['fecha']."'
					AND color = 'am' AND fecha<='".$_GET['temporadamayor']."/06/30' AND fecha>='".($_GET['temporadamayor']-1)."/09/01'
					GROUP BY color";
...
include ('../../php/conexion.php');
 $NroRegistros=mysql_num_rows(mysql_query("SELECT p.fecha, p.caplocal, p.capvisitante, p.local, p.visitante, p.caplocal, p.capvisitante, p.tgolam, p.tgolaz, p.tamam, p.tamaz, p.troam, p.troaz
					FROM partido p
					LEFT JOIN incidencias i ON p.fecha = i.fecha
					WHERE i.usuario = '".$_GET['uskey']."' AND i.fecha<='".$_GET['temporadamayor']."/06/30' AND i.fecha>='".($_GET['temporadamayor']-1)."/09/01'
					ORDER BY p.fecha",$conexion));
 ...
 if($Res>0) $PagUlt=floor($PagUlt)+1;
	...
	<?php 
	 //desplazamiento
 echo "<p style='text-align:center;'><a onclick=\"Pagina('1','".$_GET['uskey']."','".$_GET['temporadamayor']."')\" style='text-decoration:underline;cursor:pointer;color:white;'>Primero</a> ";
 if($PagAct>1) echo "<a onclick=\"Pagina('$PagAnt','".$_GET['uskey']."','".$_GET['temporadamayor']."')\" style='text-decoration:underline;cursor:pointer;color:white;'>Anterior</a> ";
 echo "<strong style='color:white;'>Pagina ".$PagAct."/".$PagUlt." </strong>";
 if($PagAct<$PagUlt)  echo " <a onclick=\"Pagina('$PagSig','".$_GET['uskey']."','".$_GET['temporadamayor']."')\" style='text-decoration:underline;cursor:pointer;color:white;'>Siguiente</a> ";
 echo "<a onclick=\"Pagina('$PagUlt','".$_GET['uskey']."','".$_GET['temporadamayor']."')\" style='text-decoration:underline;cursor:pointer;color:white;'>&Uacute;ltimo</a></p>";
