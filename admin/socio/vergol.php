<html>
<head>
<title> Ver goles de un jugador</title>
...
<?php session_start() ?>
	...
	$instruccion = "SELECT SUM( golaz ) AS golaz, SUM( golam ) AS golam
					FROM incidencias i
					WHERE usuario ='".$_GET['uskey']."' AND i.fecha<='".$_GET['temporadamayor']."/06/30' AND i.fecha>='".($_GET['temporadamayor']-1)."/09/01'";
	$consulta = mysql_query ($instruccion, $conexion)
...
<?php
					...
					FROM incidencias i
					LEFT JOIN partido p ON i.fecha = p.fecha
					WHERE i.usuario = '".$_GET['uskey']."' AND (
					i.golam >0
					OR i.golaz >0
					)AND i.fecha<='".$_GET['temporadamayor']."/06/30' AND i.fecha>='".($_GET['temporadamayor']-1)."/09/01' ORDER BY i.fecha DESC" ;
					$consulta = mysql_query ($instruccion, $conexion)
						or die ("Fallo en la consulta");
					$nfilas = mysql_num_rows ($consulta);
if ($nfilas==0) {
	?>
	<h2 style="text-align:center;margin-top:150px;margin-bottom:230px;color:AntiqueWhite;">Éste jugador aún no ha marcado ésta temporada</h2>
	<?php
