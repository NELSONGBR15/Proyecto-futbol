...
<div id="topimg">
  ...
  <div id="maincontent">
	...
	<div id="identificacion">
	  <a href="../../index.php?desconectar=1" >Desconectar</a>
        ...
        <ul class="clsClearFixSub">
		  ...
		  <li class="clsActive"><a href="./mensajes.php"><img src="../../images/socio/6.png"/>Mensajes</a></li>
		</ul>
      </div>
    </div>
    <div id="contenido">
	...
	<div id="left">
		<form action="" method="post" style="border:1px solid yellow;width:460px;margin:20px auto auto auto;padding:30px;">
	<table>
	<tr>
	<td>
	<label for="socios">Destinatario: </label>
	</td>
	<td>
	<select name="socios">
	...
	<?php include ('../../php/conexion.php');
					mysql_set_charset('utf8');
					$instruccion = "SELECT usuario,nombre,apellidos FROM socio ORDER BY activo ASC,apellidos ASC,nombre ASC " ;
					$consulta = mysql_query ($instruccion, $conexion)
						or die ("Fallo en la consulta");
					$nfilas = mysql_num_rows ($consulta);
					...
					for ($i=0; $i<$nfilas; $i++)
										...
										$resultado = mysql_fetch_array ($consulta);
										?>
	<option value= <?php echo $resultado['usuario']; ?> > <?php echo $resultado['nombre']." ".$resultado['apellidos']; ?></option>
	<?php } ?>
	</select>
	</td>
	<tr>
	...
	<td>
	<textarea name="mensaje" id "mensaje" cols="45" style="resize:vertical;padding:10px;"></textarea>
	</td>
	</tr>
	<tr>
	<td></td>
	<td><input type="submit" name="enviar" value="enviar mensaje"/>
	<input type="reset" value="Borrar contenido del mensaje"/></td>
	</tr>
	</table>
	</form>
	<?php if(isset($_POST['enviar']))
					{ 
					if ($_POST['socios']=='0'||$_POST['mensaje']=='') { ?>
					<h2 style="color:red;text-align:center">No Seleccionaste destinatario o intentaste mandar el mensaje en blanco</h2>
					<?php }
					
					else {
					include ('../../php/conexion.php');
					mysql_set_charset('utf8');
					$instruccion = "INSERT INTO mensajerec (usuario,destinatario,mensaje) VALUES('".$_SESSION['usu']."','".$_POST['socios']."','".$_POST['mensaje']."') " ;
					$consulta = mysql_query ($instruccion, $conexion)
						or die ("Fallo al mandar el mensaje");
					?>
					<h2 style="color:lightgreen;text-align:center">Mensaje enviado</h2>
					<?php }
