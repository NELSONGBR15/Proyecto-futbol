...
function Pagina(nropagina){
 ...
 ajax.open("GET", "./consultaenviados.php?pag="+nropagina);
...
fila.style.display="table-row";
 ...
 //indicamos el archivo que realizará el proceso de paginar
 //junto con un valor que representa el nro de pagina
 ajax.open("GET", "./marcarmensaje1.php?a=1&marcar="+mensajje);
 //como hacemos uso del metodo GET
 //colocamos null ya que enviamos 
 //el valor por la url ?pag=nropagina
 ajax.send(null)
...
$nfilas = mysql_num_rows ($consulta);					
					if ($nfilas==0)
						{
						echo "<h2 style='color:red;'>Aún no has enviado ningún mensaje.</h2>";
						}
						...
						echo "
							...
							<th>A</th>
							<th>Mensaje</th>
							<form action='./enviados.php' name='f1' method='post'>
						...
						for ($i=0; $i<$nfilas; $i++)
										...
										<tr>
										<td>".$fecha."</td>
										<td class='usuario'>".$resultado['destinatario']."</td>
										<td>";
										?>
										<a onclick="vermensaje('<?php echo $resultado['idmen']; ?>')" style="cursor:pointer;font-weight:bold;color:GreenYellow;"><?php echo substr($resultado['mensaje'],0,50)?>...</a>
										 <?php 
