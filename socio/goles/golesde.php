...
<div id="topimg">
  ...
  <div id="maincontent">
	...
	<div id="left">
	<div id="izqda" style="width:203px;height:50px;margin-top:50px;">
	<p>Selecciona</p>
	<?php
		include ('../../php/conexion.php');
		mysql_set_charset('utf8');
		$instruccion = "SELECT usuario, nombre, apellidos
						FROM socio
						WHERE activo = 's'
						ORDER BY apellidos" ;
		$consulta = mysql_query ($instruccion, $conexion);
		...
		{
			$pos = strpos($registro[2],' ');
			$cadena=substr($registro[2],0,$pos);
			echo "<option value='".$registro[0]."'>".$registro[1]." ".$cadena."</option>";
		}
		echo "</select>";
?>
	</div>
	<div  id="drcha" style="width:377px;">
	</div>
	</div>
	</div>
	</div>
